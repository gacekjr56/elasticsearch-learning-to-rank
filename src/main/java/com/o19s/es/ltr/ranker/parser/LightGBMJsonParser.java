...
public class LightGBMJsonParser implements LtrRankerParser {
    ...
    public NaiveAdditiveDecisionTree parse(FeatureSet set, String model) {
        ...
        try (XContentParser parser = JsonXContent.jsonXContent.createParser(
                NamedXContentRegistry.EMPTY, LoggingDeprecationHandler.INSTANCE, model)) {
        ...
        return new NaiveAdditiveDecisionTree(trees.toArray(new NaiveAdditiveDecisionTree.Node[0]), weights,
                set.size(), Normalizers.get(Normalizers.NOOP_NORMALIZER_NAME));
    }
    ...
    private static class SplitParserState {
        ...
        private static final Split.Comparer COMPARER_ZERO_DEFAULT_LEFT =
                (threshold, fval) -> fval <= threshold || isZero(fval) || Float.isNaN(fval);
        private static final Split.Comparer COMPARER_ZERO_DEFAULT_RIGHT =
                (threshold, fval) -> fval <= threshold && !isZero(fval) && !Float.isNaN(fval);
        private static final Split.Comparer COMPARER_DEFAULT_LEFT =
                (threshold, fval) -> fval <= threshold || Float.isNaN(fval);
        private static final Split.Comparer COMPARER_DEFAULT_RIGHT =
                (threshold, fval) -> fval <= threshold && !Float.isNaN(fval);
        private static final Split.Comparer COMPARER_SIMPLE = (threshold, fval) -> fval <= threshold;

        public Split.Comparer numericalDecision() {
            if (MissingType.None.equals(missingType) ||
                    (MissingType.Zero.equals(missingType) && defaultLeft && ZERO_THRESHOLD < threshold)) {
